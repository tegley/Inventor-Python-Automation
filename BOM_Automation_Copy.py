...
def main(): #Control Inventor and execution of functions
	global inventor, qty_list, print_this #Define inventor (application object) and various lists as a global variables that can be used anywhere in the code	
	inventor = wc.GetActiveObject('Inventor.Application') #Connect Inventor to Python

	#List to determine whether the quantity is 'EA' or 'LF' - # = LF item; 0.0 means EA item
	qty_list = []
	
	#List to determine the rows of the BOM that are to be printed on the material requisition
	print_this = []
	...
	if exists == True: #Running code just for the iProperties (adding parts)
		Edit_iProperties(assembly_document) #Execute Step 1
		bom = Create_BOM(drawing_document) #Execute Step 2
		Update_List(bom) #Execute Step 3
		Update_Material_Requisition(bom, path) #Execute Step 4
...
def Edit_iProperties(assembly_document): #Access parts from an assembly file and update their individual iProperties
	...
	parts_in_assembly = assembly_document.AllReferencedDocuments #Locate all referenced documents (parts and sub-assemblies used in the assembly)
	...
	for x in range(1, number_of_parts): #Iterate through all documents referenced in the assembly
		...
		if ".ipt" in current_part_name:
			...
			date_checked = inventor_design_tracking_properties.Item("Date Checked") #Change the date checked to today's date
			...
			#Pull description text box and recognize important unicode characters
			description = inventor_design_tracking_properties.Item("Description")
			inch = "\u0022" #Unicode for "

			document_subtype_name = inventor_design_tracking_properties.Item("Document SubType Name").Value #Distingish between content library parts and Angel's sheetmetal parts
			...
			if "Layout_1" in part_words_casefold: #Skip over the 'Layout_1' part
				...
				if "pipe" in part_words_casefold: #Update description of pipe in accordance to PCL standards; LF item
				...
				elif "flange" in part_words_casefold: #Update description of flanges in accordance to PCL standards; EA item
				...
				elif "L" in part_words: #Update description of angle in accordance to PCL standards; LF item
				...
				elif "Smart_Grating" in part_words: #Identify grating parts and enable the user to enter the final sizing accordingly
					add = 0.0
					qty_list.append(add)
				...
				if "plate" in part_words_casefold or "tab" in part_words_casefold or "toeboard" in part_words_casefold: #Update description for current part if it is a plate or angle tab
					...
					print("Unique part for Angel recognized: Code not yet developed")
...
def Update_List(bom):		
	...
	qty_list.reverse()
	initial_list_length = len(qty_list)
	
	print("Initial list (rounded):", qty_list)

	for qty_update in range(0, initial_list_length): #Update list to reflect item quantity
		current_item_qty = bom.PartsListRows.Item(qty_update + 1).Item(2).Value #Value of the QTY column in the BOM
		qty_list[qty_update] *= int(current_item_qty)
	
	print("Multiplied list:", qty_list)

	for list_position in range(0, initial_list_length): #Mark identical items of different lengths for deletion and identify BOM rows to be printed
		#Determine description of previous row
		current_description = bom.PartsListRows.Item(list_position + 1).Item(3).Value #Item Description in the BOM
		current_material = bom.PartsListRows.Item(list_position + 1).Item(4).Value #Material cell in the BOM
		
		Scan_and_Update(bom, list_position, current_description, current_material, initial_list_length)
		
	for i in range(0, initial_list_length): #Remove consolidated items
		try:
			if qty_list[i] == -999.123:
					del qty_list[i]
		except:
			break
				
	final_list_length = len(qty_list) #Identify length of list that has already been consolidated

	for r in range(0, final_list_length): #Round pipe length to the nearest eigth
		dummy = qty_list[r]
		rounded_length = Round_To_Nearest_Eighth(dummy) #Round pipe length to nearest eighth
		qty_list[r] = rounded_length
					
	print("Finalized list:",qty_list)
...
def Update_Material_Requisition(bom, path):
	...
	total_bom_rows_to_be_printed = len(print_this)
	row = 25

	for counter in range(0, total_bom_rows_to_be_printed):
		...
		if print_this[counter] == 0: #Don't print rows that are consolidated
			continue
		
		else: #Print consolidated item according to the first iteration listed on the BOM row
			if qty_list[counter] != 0.0: #Account for situations (pipe) where items are purchased by the linear foot
				quantity_cell_bom = qty_list[counter] #Quantity
				...
				quantity_cell_bom = float(bom.PartsListRows.Item(print_this[counter]).Item(bom_column).Value) #Quantity
				requisition[req_cell].value = (quantity_cell_bom)
			...
			description_cell_bom = bom.PartsListRows.Item(print_this[counter]).Item(bom_column).Value #Item Description cell in Requisition
			...
			material_cell_bom = bom.PartsListRows.Item(print_this[counter]).Item(bom_column).Value #Mat Spec cell in Requisition
...
def Scan_and_Update(bom, list_position, current_description, current_material, list_length):
	start = list_position + 1
	for update in range(start, list_length): #Configure
			next_description = bom.PartsListRows.Item(update + 1).Item(3).Value
			next_material = bom.PartsListRows.Item(update + 1).Item(4).Value		
			if next_description == current_description and next_material == current_material and qty_list[update] != -999.123:
				qty_list[list_position] += qty_list[update] #Add
				qty_list[update] = -999.123 #Mark for deletion
			else:
				continue
	if qty_list[list_position] != -999.123: #Mark rows to be printed
		print_this.append(start)	
